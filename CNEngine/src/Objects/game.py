...
class Game:
    ...
    def show(self: object) -> None:
        ...
        if not self.fullscreen:
          self.screen = display.set_mode(self.size, DOUBLEBUF)
        else:
            if not self.resize:
                self.screen = display.set_mode(self.size, FULLSCREEN|SCALED|DOUBLEBUF)
            else:
                self.screen = display.set_mode((I.current_w, I.current_h), FULLSCREEN|DOUBLEBUF)
